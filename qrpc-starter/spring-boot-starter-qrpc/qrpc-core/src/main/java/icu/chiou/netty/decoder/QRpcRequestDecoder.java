package icu.chiou.netty.decoder;
...
public class QRpcRequestDecoder extends LengthFieldBasedFrameDecoder {
    ...
    private Object decodeFrame(ByteBuf byteBuf) {
        ...
        if (payload != null && payload.length != 0) {
            ...
            byte compressCode = qRpcRequest.getCompressType();
            if (compressCode == 1) {
                compressor = CompressionFactory.get("gzip");
            } else {
                compressor = CompressionFactory.get("gzip");
            }
            ...
            byte serializeCode = qRpcRequest.getSerializeType();

            switch (serializeCode) {
                case 1:
                    serializer = SerializationFactory.get("jdk");
                    break;
                case 2:
                    serializer = SerializationFactory.get("hessian");
                    break;
                case 3:
                    serializer = SerializationFactory.get("json");
                    break;
                default:
                    serializer = SerializationFactory.get("jdk");
                    break;
            ...
            //HashMap<String, String> hashMap = serializer.deserialize(payload, HashMap.class);
