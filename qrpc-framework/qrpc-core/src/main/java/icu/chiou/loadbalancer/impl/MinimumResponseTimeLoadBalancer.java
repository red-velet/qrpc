package icu.chiou.loadbalancer.impl;

import icu.chiou.QRpcBootstrap;
import icu.chiou.loadbalancer.AbstractLoadBalancer;
import icu.chiou.loadbalancer.Selector;
...
public class MinimumResponseTimeLoadBalancer extends AbstractLoadBalancer {
    ...
    private static class MinimumResponseTimeSelector implements Selector {
        ...
        public InetSocketAddress getNext() {
            Map.Entry<Long, Channel> entry = QRpcBootstrap.ANSWER_TIME_CHANNEL_CACHE.firstEntry();
            if (entry != null) {
                return (InetSocketAddress) entry.getValue().remoteAddress();
            }

            // 直接从缓存中获取一个可用的就行了
            Channel channel = (Channel) QRpcBootstrap.CHANNEL_CACHE.values().toArray()[0];
